...
export function orchestratorBriefing(nodeSummary: string): string {
  return [
    ...
    "- A dispatch says four things: the **deliverable** that must come back, the **scope** to work in, how to **verify** it, and any **context** the worker cannot discover. The Host writes the brief from those, so they are not optional — a dispatch with no way to check it is refused before a machine is spent on it.",
    ...
    '- `verify` is the part most often waved away. "check it works" is not a check; "run npm test -- auth and report the failures" is.',
