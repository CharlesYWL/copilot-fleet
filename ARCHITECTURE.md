...
- **Login flows.** Authorization code + PKCE uses the Microsoft corporate tenant and the same Visual Studio public client used by KYC, with a hostless `http://localhost:<port>/` callback. Normal use needs no tenant/client setup; environment values are advanced overrides. Device code is an optional fallback that stays off until an administrator has watched one complete on this Host. MSAL performs all protocol and token validation; Fleet hand-rolls no JWT checking and persists no Microsoft token.
...
- **High-impact actions** — removing an administrator, enabling or disabling the password, minting an enrollment grant, exporting a portable backup — additionally require an authorization-code sign-in within ten minutes. A device sign-in does not satisfy it: an attacker can start a device flow and have an administrator finish it.
